...
void ffprintf(FILE *of, Function *func, const char *ty) {
    if (!of || !func || !ty) {
        return;
    }
    //Get the file where the function resides in.
...
void process_netdev_st(GlobalVariable *currGlobal, FILE *outputFile) {
    ...
    if(currGlobal->hasInitializer()) {
        ...
        if(targetFunction != nullptr && !targetFunction->isDeclaration()) {
            ffprintf(outputFile, targetFunction, NETDEV_IOCTL);
        }
    }
}
...
void process_device_attribute_st(GlobalVariable *currGlobal, FILE *outputFile) {
    ...
    if(currGlobal->hasInitializer()) {
        ...
        if(targetFunction != nullptr && !targetFunction->isDeclaration()) {
            ffprintf(outputFile, targetFunction, DEVATTR_SHOW);
        }
        ...
        if(targetFunction != nullptr && !targetFunction->isDeclaration()) {
            ffprintf(outputFile, targetFunction, DEVATTR_STORE);
        }
...
void process_file_operations_st(GlobalVariable *currGlobal, FILE *outputFile) {
    ...
    if(currGlobal->hasInitializer()) {
        ...
        if (actualStType->getNumOperands() >= 3) {
            ...
            if(targetFunction != nullptr && !targetFunction->isDeclaration()) {
                ffprintf(outputFile, targetFunction, READ_HDR);
            }
        }

        // write: 3
        if (actualStType->getNumOperands() >= 4) {
            Value *currFieldVal = actualStType->getOperand(3);
            Function *targetFunction = dyn_cast<Function>(currFieldVal);
            if(targetFunction != nullptr && !targetFunction->isDeclaration()) {
                ffprintf(outputFile, targetFunction, WRITE_HDR);
            }
        }

        // ioctl
        for (unsigned i = 4; i < actualStType->getNumOperands(); ++i) {
            Value *currFieldVal = actualStType->getOperand(i);
            Function *targetFunction = dyn_cast<Function>(currFieldVal);
            if(targetFunction != nullptr && !targetFunction->isDeclaration()) {
                std::string fn = targetFunction->getName().str();
                if (fn.find("ioctl") != std::string::npos) {
                    ffprintf(outputFile, targetFunction, IOCTL_HDR);
                }
            }
        }
    }
...
void process_v4l2_ioctl_st(GlobalVariable *currGlobal, FILE *outputFile) {
    if(currGlobal->hasInitializer()) {
        ...
        for(unsigned int i=0; i<actualStType->getNumOperands(); i++) {
            ...
            Function *targetFunction = dyn_cast<Function>(currFieldVal);
            if(targetFunction != nullptr && !targetFunction->isDeclaration()) {
                ffprintf(outputFile, targetFunction, V4L2_IOCTL_FUNC);
            }
        }
    ...
    if(currGlobal->hasInitializer()) {
        ...
        for(auto curre:allentries) {
            if(curre.find(curr_st_name) != std::string::npos) {
                ...
                if(targetFunction != nullptr && !targetFunction->isDeclaration()) {
                    ffprintf(outputFile, targetFunction, tokens[2]);
                }
                if (tokens)
                {
                    int i;
...
void process_global(GlobalVariable *currGlobal, FILE *outputFile, std::vector<string> &allentries) {
    ...
    if (containedType->isStructTy()) {
        ...
        if (containedType->getStructName() == file_op_st || containedType->getStructName().find(file_op_st + ".") == 0) {
            ...
            while(kernel_sts[++i].method_lab != END_HDR) {
                if(kernel_sts[i].st_name == containedType->getStructName()) {
                    if(currGlobal->hasInitializer()) {
                        Constant *targetConstant = currGlobal->getInitializer();
                        ConstantStruct *actualStType = dyn_cast<ConstantStruct>(targetConstant);
                        if (actualStType == nullptr || kernel_sts[i].mem_id >= actualStType->getNumOperands()) {
                            continue;
                        }
                        ...
                        if(targetFunction != nullptr && !targetFunction->isDeclaration()) {
                            ffprintf(outputFile, targetFunction, kernel_sts[i].method_lab.c_str());
                        }
                    }
                }
            }
...
int main(int argc, char *argv[]) {
    ...
    llvm::Expected<std::unique_ptr<llvm::Module>> moduleOrErr = parseBitcodeFile(fileOrErr.get()->getMemBufferRef(), context);
    ...
    if (!m) {
        std::cerr << "Fail to get the module!!\n";
        return -1;
    }
    //Structure based entry function identification.
    ...
    for (Module::global_iterator gstart = currGlobalList.begin(), gend = currGlobalList.end(); gstart != gend; gstart++) {
        ...
        //std::cerr << "GV: " << (currGlobal->hasName() ? currGlobal->getName().str() : "") << "\n";
        process_global(currGlobal, outputFile, entryPointStrings);
    }
    //Name based entry function identification, e.g. xxx_ioctl, as another hint
    for (Function &func : *m) {
        if (!func.hasName()) {
            continue;
        }
        std::string fn = func.getName().str();
        if (fn.size() > 5 && fn.find("_ioctl") == fn.size() - 6) {
            //See if this function has ever been explicitly called by another function, if so, exclude it since it cannot be a top-level entry. 
            bool called = false;
            ...
            if (!called) {
                ffprintf(outputFile, &func, "IOCTL_CAND");
            }
        }
    }
    /*for(auto a = m->begin(), b = m->end(); a != b; a++) {
